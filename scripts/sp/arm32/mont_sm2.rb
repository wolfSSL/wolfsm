...
module MontArm32_SM2
  ...
  def add_red_sm2(a, o, t, single=false)
    ...
    sbcs(a[7], a[7], o, LSL_ARM32.new(1))
    if single
      rsb(o, o, 0)
      sbc(o, o, 0)
    else
  ...
  def sub_red_sm2(a, o, t)
    ...
    adcs(a[7], a[7], o, LSL_ARM32.new(1))
    adc(o, o, 0)
...
EOF
    ...
    adcs(a[1], a[1], mask)
    adcs(a[2], a[2], mask)
    adcs(a[3], a[3], mask, LSL_ARM32.new(1))
    mov(o, 0)
    adc( o   , o   , 0)
    ...
    lsr(l[2], a[2], 1)
    lsr(l[3], a[3], 1)
    orr(l[0], l[0], a[1], LSL_ARM32.new(31))
    orr(l[1], l[1], a[2], LSL_ARM32.new(31))
    orr(l[2], l[2], a[3], LSL_ARM32.new(31))
    orr(l[3], l[3], o   , LSL_ARM32.new(31))
    mov(o, a[0])
    strd(l[0], l[1], @r[4])
    strd(l[2], l[3], @r[6])
...
EOF
  ...
  def mont_mul_sm2_no_umull(words)
    puts <<EOF
...
EOF
    ...
    m0 = use_reg(r6)
    m1 = use_reg(r7)
    out = [ use_reg(r8), use_reg(r9), use_reg(r10), use_reg(lr) ] * 4
    t3 = use_reg(r12)
...
EOF
    ...
    0.upto(words * 2 - 1) do |k|
      0.upto(words - 1) do |i|
        ...
        mul_op(ii, jj, @a, @b, c, o, out, li, lj)

        li = ii
        lj = jj
      end
      str(out[k], tmp[k]) if k < words+words/2
    end
    (words+words/2).upto(2*words-1) do |i|
      str(out[i], tmp[i])
    end

    str(@r, rsp)

    r = do_mont_red_sm2(words, sp, sp, @a, @b, t3)
...
EOF
  ...
  def mont_mul_sm2(words)
    puts "#if defined(WOLFSSL_ARM_ARCH) && (WOLFSSL_ARM_ARCH < 4)"
    @arm_arch_less_than_4 = true
    mont_mul_sm2_no_umull(words)
    @arm_arch_less_than_4 = false
    puts "#elif defined(WOLFSSL_ARM_ARCH) && (WOLFSSL_ARM_ARCH < 7)"
  ...
  def mont_sqr_sm2_no_umull(words)
    puts <<EOF
...
EOF
    ...
    t3 = use_reg(r12)
    out = [ use_reg(r8), use_reg(r9), use_reg(r10), use_reg(lr) ] * 4
    out2 = [ r3, r4, r6, r7, r8, r9, r10, lr, r12 ] * 2
...
EOF
    ...
    1.upto(2*words-3) do |k|
      ...
      if k[0] == 0 and k >= 4 and k <= words * 2 - 6
        order[k] = order[k].reverse
      end
    end
    1.upto(2*words-3) do |k|
      order[k].each do |i, j|
        sqr_op(i, j, @a, c, o, out, li, lj)
    ...
    str(out[2*words-2], tmp2[2*words-2])

    u = 9
    commenta("Double")
    1.step(2*words-2, u) do |k|
      max = 2*words-2 - k
      max = u-1 if max > u-1
      0.upto(max) do |i|
        ldr(out2[i+k], tmp2[i+k])
      end
      adds(out2[0+k], out2[0+k], out2[0+k]) if k == 1
      adcs(out2[0+k], out2[0+k], out2[0+k]) if k != 1
      1.upto(max) do |i|
        adcs(out2[i+k], out2[i+k], out2[i+k])
      end
      0.upto(max) do |i|
        str(out2[i+k], tmp2[i+k])
      end
    end
    adc(out2[2*words-1], t2, 0)
    str(out2[2*words-1], tmp2[2*words-1])

    # Squares
    o2 = []
    c2 = []
    0.step(words-1,2) do |i|
        ldr(t0, tmp2[i*2+0]) if i != 0
        ldr(t1, tmp2[i*2+1])
        ldr(t2, tmp2[i*2+2])
        ldr(t3, tmp2[i*2+3])
        sqr_op(i, i, @a, c2, o2, out, -1, -1)
        sqr_op(i+1, i+1, @a, c2, o2, out, -1, -1)
        if i == 0
          adds(out[i*2+1], out[i*2+1], t1)
        else
          puts "#if !(defined(WOLFSSL_ARM_ARCH) && (WOLFSSL_ARM_ARCH < 4))"
          adcs(out[i*2], out[i*2], t0)
          adcs(out[i*2+1], out[i*2+1], t1)
          puts "#else"
          adds(out[i*2], out[i*2], t0)
          adcs(out[i*2+1], out[i*2+1], t1)
          puts "#endif"
        end
        adcs(out[i*2+2], out[i*2+2], t2)
        adcs(out[i*2+3], out[i*2+3], t3) if i != words-2
        adc( out[i*2+3], out[i*2+3], t3) if i == words-2
        str(out[i*2+0], tmp[i*2+0])
        str(out[i*2+1], tmp[i*2+1])
        str(out[i*2+2], tmp[i*2+2])
        str(out[i*2+3], tmp[i*2+3])
    end

    str(@r, rsp)

    r = do_mont_red_sm2(words, sp, sp, @a, t4, t3)
...
EOF
  ...
  def do_mont_red_sm2(words, tmp, out, t1, t2, t3)
    ...
    i_sub( t[7], t[7], t[0], LSL_ARM32.new(2))
    commenta "   + (a[0..1] * 2) << (6 * 32)"
    adds(t[6], t[6], t[0], LSL_ARM32.new(1))
    mov(r[2], t[5])
    adc( t[7], t[7], t[1], LSL_ARM32.new(1))
    i_add( t[7], t[7], t[0], LSR_ARM32.new(31))
    commenta "   - (a[0..2] * 2) << (5 * 32)"
    subs(t[5], t[5], t[0], LSL_ARM32.new(1))
    sbcs(t[6], t[6], t[1], LSL_ARM32.new(1))
    sbc( t[7], t[7], t[2], LSL_ARM32.new(1))
    subs(t[6], t[6], t[0], LSR_ARM32.new(31))
    mov(r[1], t[4])
    sbc( t[7], t[7], t[1], LSR_ARM32.new(31))
    commenta "   + a[0..3] << (4 * 32)"
    adds(t[4], t[4], t[0])
    adcs(t[5], t[5], t[1])
    adcs(t[6], t[6], t[2])
